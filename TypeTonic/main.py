...
class PersonalCabinet:
    ...
    def show_acc(self):
        ...
        self.top_problematic_letters_button = tk.Button(
            self.account_frame,
            text="Топ проблемных букв",
    ...
    def show_last_attempts(self):
        ...
        for i, attempt in enumerate(attempts, start=1):
            ...
            table.insert(
                parent="", index="end", iid=i, text=str(i), values=(cps, cpm, accuracy)
            )
    ...
    def show_problematic_letters(self):
        ...
        for i, attempt in enumerate(attempts, start=1):
            ...
            table.insert(
                parent="", index="end", iid=i, text=str(i), values=(letter, cnt)
            )
...
class LoginPanel:
    ...
    def show_login_screen(self):
        ...
        tk.Label(self.login_frame, text="Username:", font=("Helvetica", 14)).grid(
            row=0, column=0, sticky="e"
        )
        tk.Entry(
            self.login_frame, textvariable=self.username_var, font=("Helvetica", 14)
        ).grid(row=0, column=1)

        tk.Label(self.login_frame, text="Password:", font=("Helvetica", 14)).grid(
        ...
        tk.Button(
            self.login_frame, text="Войти", command=self.login, font=("Helvetica", 14)
...
class TrainingModePanel:
    ...
    def show_training_mode_page(self, mode):
        ...
        tk.Label(self.training_frame, text="TypeTonic!", font=("Helvetica", 14)).grid(
            row=0, column=1, columnspan=2, pady=10
        )
        tk.Label(
            self.training_frame, text="Выберете сложность:", font=("Helvetica", 14)
        ...
        tk.Label(
            self.training_frame, text="Выберете язык:", font=("Helvetica", 14)
        ...
        user_button = tk.Button(
            self.training_frame,
            text="Аккаунт",
            command=self.show_cabinet_panel,
            font=("Helvetica", 14),
        )
...
class TrainingPanel:
    ...
    def show_training(self, mode):
        ...
        self.sample_label = tk.Label(
            self.frame, text=self.get_random_text(mode), font=("Helvetica", 18)
        )
        ...
        self.reset_button = tk.Button(
            self.frame,
            text="Еще раз",
            command=lambda: self.reset(mode),
            font=("Helvetica", 24),
        )
        ...
        self.exit_button = tk.Button(
            self.frame, text="Выход", command=self.end_training, font=("Helvetica", 24)
        )
    ...
    def start(self, event):
        """
        Функция проверяет, если тренировка еще не запущена, то запускает ее.
        Иначе, проверяет введенный текст на корректность. Если весь текст
        введен корректно, то функция останавливается
        :param event:
        """
        if not self.running:
    ...
    def time_thread(self):
        """
        Функция производит отсчет времени, прошедшего в тренировке. Пересчитывает
        колличество ошибок и скорость пользователя. По завершению, отправляет данные на сервер
        """
        while self.running:
            time.sleep(0.1)
            self.counter += 0.1
    ...
    def get_random_text(self, mode):
        ...
        with open(
                "languages/" + mode["language"] + "_" + str(mode["difficulty"]) + "k.json"
        ) as file:
            text = json.load(file)
...
class SpeedTypingApp:
    ...
    def __init__(self, root):
        ...
        self.account = {"nickname": "Noname"}
        """Информация о текущем пользователе"""

        self.mode = {"difficulty": 2, "language": "Русский"}
