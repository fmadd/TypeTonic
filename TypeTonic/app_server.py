...
class app_server:
    ...
    def valid_password(self, password):  # временно закоменчено
        """
        Функция проверяет надежность пароля используя соответствующее
    ...
    def log_in(self, login, password):
        """
        Функция проверяет может ли пользователь залогиниться в системе с
        данными логином и паролем
        :param str login:
        :param str password:
        :rises: Exception("Слишком много попыток ввода логина"),
    ...
    def reg_user(self, login, password):
        """
        Функция регистрирует пользователя с заданным логином и
        паролем, если таких пользователей еще нет в базе
        :param str login:
        :param str password:
        """
    ...
    def user_dynamics(self):
        """
        Функция передает изменения в характеристиках пользователя.
        Сравниваются среднее значения за все время и последнюю попытку
        :return: Лист, в котором отражены соответсвующие изменения
        характеристик
        :rtype: list
        """
        conn = http.client.HTTPConnection(get_ip(), 2000)
    ...
    def get_user_letter(self):
        """
        Функция возвращает топ букв пользователя в которых он чаще
        всего ошибается
