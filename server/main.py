...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    """
    Обработчик методов без учета авторизации пользователя
    Наследник класса BaseHTTPRequestHandler
    """
    ...
    def do_GET(self):
        ...
        if self.path == "/stat_user_all":  # статистика одного пользователся
            ...
            self.wfile.write(res.encode())
                ...
                self.path == "/top_users_week"
        ):  # вывод топа пользователей за последнюю неделю
            self.form_response()
...
class AuthHTTPRequestHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        """
        Функция обрабатывает Post запросы клиента (на регистрацию и авторизацию)
        и возвращает код ответа
        """
