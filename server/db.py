...
def db_add_user(login, user_password, cnf=config_bd):
    ...
    with psycopg2.connect(
        ...
        if valid_name(login, cnf):
            ...
            cursor.execute(
                f"insert into users (user_login,user_password) values ( '{login}','{user_hash}')"
            )
        else:
            raise Exception("NotValidName")


def db_check_user(login, user_password, cnf=config_bd):
...
def db_add_attempt(result_json, cnf=config_bd):
    ...
    with psycopg2.connect(
        ...
        login, time, cps, cpm, acc, mistakes = (
            js["login"],
            js["curr_data"],
            js["cps"],
            js["cpm"],
...
def db_get_top_letters(cnf=config_bd):
    ...
    with psycopg2.connect(
        ...
        connect.autocommit = True
        cursor.execute(
            f"select letter, sum(count) as cnt from mistakes group by letter order by cnt DESC "
        )
        return cursor.fetchall()
...
def db_user_all(login, cnf=config_bd):
    ...
    with psycopg2.connect(
        ...
        connect.autocommit = True
        cursor.execute(
            f"select avg(cps), avg(cpm), avg(acc) from attempts where login='{login}' group by login"
        )
        return cursor.fetchall()
